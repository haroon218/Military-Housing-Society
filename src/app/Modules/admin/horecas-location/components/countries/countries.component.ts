...
@Component({
  ...
  standalone: true,
  imports: [FormsModule,TranslateModule,TableModule,CommonModule,BreadcrumbModule,DialogModule,Footer,ReactiveFormsModule,ButtonModule, DropdownModule],
...
this.loadCountries()
  ...
  onAddOrUpdateRegion(): void {
    ...
    const apiCall = this.isEditMode 
      ? this.locationService.updateCountry(this.selectedCountry.id, countryData) 
      : this.locationService.addCountry(countryData);
    ...
    this.loadCountries(this.perPage,event.value)
    }
    loadCountries(per_page?:any,region_id?:any): void {
      this.loading = true;
      debugger
      this.locationService.getCountries(per_page,region_id).subscribe({
        next: (response: any) => {
          debugger
    ...
    confirmDelete() {
      if (this.countryIdToDelete !== null) {
        ...
        this.locationService.deleteCountry(this.countryIdToDelete).subscribe({
          next: () => {
