...
@Component({
  ...
  imports: [TableModule,DialogModule, CommonModule, InputTextModule, TagModule, FormsModule,BreadcrumbModule,
    DropdownModule, MultiSelectModule, ProgressBarModule, ButtonModule, AdminModule,RouterModule,ReactiveFormsModule],
...
constructor(private toastr:ToastrService,private fb:FormBuilder,private service:CompaignsService,private companyService: AuthService) {}
...
onAddGiveaway() {
  ...
  formData.append('points', this.giveawayForm.value.points);
...
updateOffer() {
  ...
  this.service.updatereward(this.offerId, formData).subscribe({
    next: (response) => {
...
this.loading=true
  this.service.getRewards(queryParams).subscribe({
    next: (response: any) => {
      debugger
      this.loading=false
...
addGiveawayOffer(formData: FormData) {
  this.service.addReward(formData).subscribe({
    next: (response) => {
      this.display = false;
      this.isLoading = false;
      this.loadingText = '';
...
confirmDelete() {
  if (this.employeeIdToDelete !== null) {
    ...
    this.service.deleteReward(this.employeeIdToDelete).subscribe({
      next: (response:any) => {
        this.toastr.success(response.message, 'Success');
